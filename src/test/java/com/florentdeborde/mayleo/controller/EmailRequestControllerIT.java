...
class EmailRequestControllerIT {
        ...
        void should_return_413_when_payload_is_too_large() throws Exception {
                // We create a very large string to exceed the 256KB limit
                // 1 char = 1 byte in standard JSON, so 300,000 chars > 256 * 1024
                String hugeMessage = "A".repeat(300 * 1024);
                ...
                // The filter checks the size BEFORE HMAC, so we don't even need a valid
                // signature
                mockMvc.perform(post(urlTemplate)
                                .header(headerKeyName, PLAIN_API_KEY)
                                .header("Origin", allowedDomain)
        ...
        void should_sanitize_html_in_subject_and_message() throws Exception {
                ...
                // THEN: Verify in Database (we can check the repository because we are
                // @Transactional)
                // Since we don't return the full object, we fetch it by subject (or just get
                // the last one if we cleaned up)
                // Ideally we should extract the ID from response, but here let's assume it's
                // the only one created in this transaction context context
